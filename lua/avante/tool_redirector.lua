...
local function get_default_server(tool_name)
  ...
  if not redirection then return nil end
  ...
  if is_mcphub_available() then
    ...
    for _, server in ipairs(servers) do
      if server.name == redirection.mcp_server then return redirection.mcp_server end
    end

    -- Fallback to first available server that might support the tool
    if #servers > 0 then return servers[1].name end
  end
...
function M.redirect_tool_use(tool_use)
  ...
  if not redirection then return nil, "No redirection available for tool: " .. tool_use.name end

  if not is_mcphub_available() then return nil, "mcphub.nvim is not available for redirection" end

  local server_name = get_default_server(tool_use.name)
  if not server_name then return nil, "No suitable MCP server available for tool: " .. tool_use.name end
...
function M.should_redirect(tool_name)
  ...
  -- Only redirect if explicitly disabled in config
  -- This prevents automatic redirection when mcphub is available
...
function M.get_redirectable_tools() return vim.tbl_keys(TOOL_REDIRECTIONS) end

-- Get redirection info for a tool
function M.get_redirection_info(tool_name) return TOOL_REDIRECTIONS[tool_name] end
